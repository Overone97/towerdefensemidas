  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-20 right-4 z-20 px-3 py-2 rounded-lg bg-card border border-border text-sm font-mono hover:bg-accent transition-colors"
      >
        📋 Quêtes {unclaimedCount > 0 && <span className="text-yellow-400 ml-1">({unclaimedCount}!)</span>}
      </button>
    );
  }
