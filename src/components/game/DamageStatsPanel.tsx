...
const DamageStatsPanel: React.FC<DamageStatsPanelProps> = ({ engine }) => {
  ...
  const [stats, setStats] = useState<ReturnType<GameEngine['getDamageStats']>>([]);
  const intervalRef = useRef<number | null>(null);

  useEffect(() => {
    if (open) {
      // Update stats every 500ms instead of every render for stability
      const update = () => setStats(engine.getDamageStats());
      update();
  ...
  const maxDmg = stats.length > 0 ? Math.max(...stats.map(s => s.waveDamage), 1) : 1;

  if (engine.towerManager.units.length === 0) return null;
  ...
  return (
    <div className="absolute right-2 bottom-2 z-10">
      ...
      {open && (
        <div className="absolute bottom-8 right-0 bg-popover/95 border border-border rounded-lg shadow-lg p-3 min-w-[220px] max-w-[280px] max-h-[300px] overflow-y-auto">
          ...
          <div className="space-y-1">
            {stats.map(s => (
              <div key={s.unitId} className="flex items-center gap-1.5">
                <CharacterSprite config={s.config} size={18} owned />
                ...
                <span className="text-[9px] font-mono text-muted-foreground w-8 text-right">{Math.floor(s.waveDamage)}</span>
              </div>
            ))}
          </div>
        </div>
